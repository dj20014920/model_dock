...
export async function fetchRequestParamsFrom(base: string) {
  const html = await ofetch(base, { responseType: 'text' })
...
export async function fetchRequestParams() {
  return fetchRequestParamsFrom('https://bard.google.com/')
}
