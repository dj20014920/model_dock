...
import { onSubmitted, registerInput, unregisterInput } from '~app/utils/manual-dispatch'
...
interface Props {
  ...
  // optional: when provided, this input can be targeted by manual-dispatch
  botId?: BotId
...
const ChatMessageInput: FC<Props> = (props) => {
  ...
  const onPaste: ClipboardEventHandler<HTMLTextAreaElement> = useCallback(
    (event) => {
      ...
      if (imageFile) {
        if (!props.supportImageInput) {
          // ignore image handling in compact mode; treat as normal paste
          return
        }
      ...
      // non-image files: no-op
    },
    [props.botId, props.supportImageInput],
  )
