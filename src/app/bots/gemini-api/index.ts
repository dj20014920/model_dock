...
export class GeminiApiBot extends AbstractBot {
  ...
  async doSendMessage(params: SendMessageParams) {
    if (!this.conversationContext) {
      // pick user-selected model (fallback to gemini-pro)
      const cfg = await getUserConfig()
      const modelName = (cfg as any).geminiApiModel || 'gemini-pro'
      const model = this.sdk.getGenerativeModel({ model: modelName })
