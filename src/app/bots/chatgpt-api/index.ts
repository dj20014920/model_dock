...
export class ChatGPTApiBot extends AbstractChatGPTApiBot {
  ...
  private getModelName() {
    ...
    // Allow explicit override via user-config custom field when present
    const maybeCustom = (this.config as any).chatgptApiCustomModel
    if (maybeCustom && typeof maybeCustom === 'string' && maybeCustom.trim()) {
      return maybeCustom.trim()
    }
